...
check_lifx_response<-function(response){
  ...
  code_meaning <- switch(as.character(status),
                         ...
                         `404` = c( "Not Found", "Selector did not match any lights.", "failure"),
                         `422` = c( "Unprocessable Entity", "Missing or malformed parameters.", "failure"),
                         `426` = c( "Upgrade Required", "HTTP was used to make the request instead of HTTPS. Repeat the request using HTTPS instead. See the Authentication section.", "failure"),
                         `429` = c( "Too Many Requests", "failure"),
                         `500` = c( "Server Error", "Something went wrong on LIFX's end.", "failure"),
                         `502` = c( "Server Error", "Something went wrong on LIFX's end.", "failure"),
                         `503` = c( "Server Error", "Something went wrong on LIFX's end.", "failure"),
                         `523` = c( "Server Error", "Something went wrong on LIFX's end.", "failure")
  )
